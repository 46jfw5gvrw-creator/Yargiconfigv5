...
export default {
  async fetch(request, env) {
    ...
    // Ana sayfadaki gÃ¶rsel Cloudflare Static Assets'tan gelir.
    if (path === "/site.png") {
...
html,body{margin:0;padding:0;width:100%;min-height:100%;background:#02060a}
body{color:#fff;font-family:Arial,sans-serif;display:block;overflow-x:hidden}
.wrap{width:100%;max-width:1536px;margin:0 auto;position:relative;line-height:0}
.wrap img{width:100%;height:auto;max-width:100%;display:block;margin:0;padding:0;border:0}
.tg{position:absolute;left:6%;top:39%;width:23%;height:6%;display:block;z-index:5}
@media (max-width:1536px){.wrap{max-width:100%}}
@media (max-width:768px){.tg{left:6%;top:39%;width:23%;height:6%}}
@media (max-width:480px){.tg{left:6%;top:39%;width:23%;height:6%}}
</style>
</head>
<body>
<div class="wrap">
...
// Panelde TÃ¼rkÃ§e karakterleri HTML entity olarak kullandÄ±k; bÃ¶ylece kaynak dosyanÄ±n
// yanlÄ±Å kodlanmasÄ± durumunda bile "KullanÄ±cÄ±", "Åifre" ve "GiriÅ Yap" bozulmaz.
...
.post div{flex:1}
@media(max-width:700px){.grid{grid-template-columns:1fr}.wrap{padding:15px}.card{padding:16px;border-radius:12px}.post{align-items:flex-start}.post button{flex:0 0 auto}.post small{display:block;margin-top:4px}button{min-height:44px}input,textarea,select{font-size:16px}}
@media(min-width:701px) and (max-width:1100px){.wrap{padding:20px}}
</style>
</head>
<body>
<div class="wrap">
  <div id="login" class="card">
    <h1>â YARGI<span class="blue">CONFIG</span></h1>
  ...
  <div id="app" class="hidden">
    <div class="card">
      ...
      <button onclick="logout()">&Ccedil;Ä±kÄ±Å</button>
    </div>

    <div class="grid">
      <div class="stat"><b id="pc">0</b>PaylaÅÄ±m</div>
      <div class="stat"><b>4.6</b>Site s&uuml;r&uuml;m&uuml;</div>
      <div class="stat"><b>â</b>Sistem aktif</div>
    </div>

    <div class="card">
      <h2>ð° Yeni PaylaÅÄ±m</h2>
      <input id="title" placeholder="Ba&#351;l&#305;k">
      <input id="version" placeholder="S&uuml;r&uuml;m (Ã¶rn. 4.6)">
      <select id="status">
        <option>Yeni</option>
      ...
      <input id="link" placeholder="Telegram / indirme baÄlant&#305;s&#305;">
...
async function load(){
  ...
  posts.innerHTML=ps.map(x=>
    '<div class="post"><div><b>'+esc(x.title)+'</b><small class="muted"> '+esc(x.version||'')+' Â· '+esc(x.status||'')+'</small></div><button class="danger" onclick="del('+x.id+')">Sil</button></div>'
